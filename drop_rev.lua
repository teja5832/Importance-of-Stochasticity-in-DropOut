...
function Proj:Rethink()
	self.noiseWeight:copy(torch.gt((self.weight-torch.mean(self.weight)):abs(), self.p*torch.std(self.weight)))
	self.noiseBias:copy(torch.gt((self.bias-torch.mean(self.bias)):abs(), self.p*torch.std(self.bias)))	
